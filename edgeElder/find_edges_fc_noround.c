...
double 	g2max1,    /* Max G2 response of correct sign */
        ...
        k_thresh= 0.1,  /* Min. contrib. to linear interp. for pixel to be labeled */
        ...
        int
                ...
                double *imgR, double *imgG, double *imgB, double *Rdark, double *Gdark, double *Bdark,
                double *Rlight, double *Glight, double *Blight, double *nxendd, double *nyendd, double *pxendd, double *pyendd)
        {
            ...
            double 	l,d;
            double  lR,dR,lG,dG,lB,dB,meanR,meanG,meanB;
            ...
            noise 		= sensor_noise;
            g2thresh 	=  5.2* 0.0000596*noise; //5.2*g2thresh*noise; /* threshold on interpolated g2 values */
...
//printf("%.4f %.4f %.4f %.4f %d \n", g2thresh, noise, *g1magd, *g2d, maxg1len);
            
            
            for (x=0;x<cols;x++)
                for (y=0;y<rows;y++,g1magd++,g1dird++,g2d++,imgd++,imgdR++,imgdG++,imgdB++) /* For each pixel in image */
                {
                ...
                if (*g1magd > 0.0 && *g2d >= 0.0) /* Non-negative 2nd derivative */
                {
                    ...
                    if (g2max1 != 0.0 && g1max) /* Gradient max and found significant negative G2 response */
                    {
                        ...
                        *(nxendd+count) = round_int(nxend);*(nyendd+count) = round_int(nyend);*(pxendd+count) = round_int(pxend);*(pyendd+count) = round_int(pyend);
                        ...
                        if ((g2max1>g2maxp) && (g2max2>g2maxn) && g1max)
                        {
                                ...
                                /* Pixel lies between min negative and max positive G2 responses */
                            {
                                *(xout+count) 	= 255;
                                ...
                                lR = *(imgdR+round_int(nxend)*rows+round_int(nyend));
                                dR = *(imgdR+round_int(pxend)*rows+round_int(pyend));
                                lG = *(imgdG+round_int(nxend)*rows+round_int(nyend));
                                dG = *(imgdG+round_int(pxend)*rows+round_int(pyend));
                                lB = *(imgdB+round_int(nxend)*rows+round_int(nyend));
                                dB = *(imgdB+round_int(pxend)*rows+round_int(pyend));
                                
                                
                                if (l<=d)
                                {
                                    meanl = (0.5*(l+d));
                                    meanR = round_int(0.5*(lR+dR));
                                    meanG = round_int(0.5*(lG+dG));
                                    meanB = round_int(0.5*(lB+dB));
                                ...
                                else if (blurvar < 1)
                                {
                                    ...
                                    *(Rdark+count)	= round_int(dR);
                                    *(Rlight+count)	= round_int(lR);
                                    
                                    *(Gdark+count)	= round_int(dG);
                                    *(Glight+count)	= round_int(lG);
                                    
                                    *(Bdark+count)	= round_int(dB);
                                    *(Blight+count)	= round_int(lB);
                                ...
                                else
                                {
                                    ...
                                    if (D<0)
                                        ...
                                        *(Rdark+count) = round_int(D);
                                    if (L<0)
                                        *(Rlight+count)= 0;
                                    else if (L> 255)
                                        *(Rlight+count) = 255.0;
                                    else
                                        *(Rlight+count) = round_int(L);
                                    
                                    C	= (lG - dG) / erf_const;
                                    D	= dG - 0.5 * C * (1 - erf_const);
                                    L	= C + D;
                                    ...
                                    if (D<0)
                                        ...
                                        *(Gdark+count) = round_int(D);
                                    if (L<0)
                                        *(Glight+count)= 0;
                                    else if (L> 255)
                                        *(Glight+count) = 255.0;
                                    else
                                        *(Glight+count) = round_int(L);
                                    
                                    C	= (lB - dB) / erf_const;
                                    D	= dB - 0.5 * C * (1 - erf_const);
                                    L	= C + D;
                                    ...
                                    if (D<0)
                                        ...
                                        *(Bdark+count) = round_int(D);
                                    if (L<0)
                                        *(Blight+count)= 0;
                                    else if (L> 255)
                                        *(Blight+count) = 255.0;
                                    else
                                        *(Blight+count) = round_int(L);
        ...
        int g2_path(double theta,int bx,int by,double *g1mag,double *g1dir,double *g1scale,
                double *g2,double *g2scale,int g2signofdouble)
        {
    ...
    from 0 in the theta direction. */
            ...
            if (g2val != 0.0 && signofdouble(g2val) == g2signofdouble) /* sig. G2 of right sign */
            {
...
#####################################*/
            while (!end_of_path && g1len < maxg1len)
            {
                ...
                else
                {
                    ...
                    if (!(g2max1==0.0 && fabs(g2val)>g2thresh && signofdouble(g2val) == g2signofdouble)   &&
                            !((same_dir1 && k1>k_thresh) || (same_dir2 && k2>k_thresh))
                            || (fabs(g2val) > g2thresh && (signofdouble(g2val) != g2signofdouble) && zcross)
                            || ((fabs(g2val) < g2thresh) && (length > acuity)))
                    {
                        end_of_path = 1;
                    }
                    else if ((signofdouble(g2val) == g2signofdouble) && (fabs(g2val) > fabs(g2max1)))
                    {
                        g2max1 = g2val;
                    ...
                    else if (((signofdouble(g2val) != g2signofdouble) || g2val == 0.0) && g2max1 != 0.0)
                    {
                        end_of_path = 1;
                    }
                    else if ((signofdouble(g2val) != g2signofdouble) && (fabs(g2val) > fabs(g2max2)))
                    {
        ...
        void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
        {
            ...
            if (nrhs < 14 )
                mexErrMsgTxt("requires  at least 14 args.");
            ...
            /* ARG 12: IMAGE R */
            arg = prhs[11];
            if notDblMtx(arg) mexErrMsgTxt("IMAGE R arg must be a real non-sparse matrix.");
            imgR = mxGetPr(arg);
            
            /* ARG 13: IMAGE G */
            arg = prhs[12];
            if notDblMtx(arg) mexErrMsgTxt("IMAGE G arg must be a real non-sparse matrix.");
            imgG = mxGetPr(arg);
            
            /* ARG 14: IMAGE B */
            arg = prhs[13];
            if notDblMtx(arg) mexErrMsgTxt("IMAGE B arg must be a real non-sparse matrix.");
            imgB = mxGetPr(arg);
            
            /*   arg = prhs[14];
    if notDblMtx(arg) mexErrMsgTxt("nxend must be a real non-sparse matrix.");
    nxend = mxGetPr(arg);
  
    arg = prhs[15];
    if notDblMtx(arg) mexErrMsgTxt("nyend must be a real non-sparse matrix.");
    nyend = mxGetPr(arg);
  
    arg = prhs[16];
    if notDblMtx(arg) mexErrMsgTxt("pxend must be a real non-sparse matrix.");
    pxend = mxGetPr(arg);
  
    arg = prhs[17];
    if notDblMtx(arg) mexErrMsgTxt("pyend must be a real non-sparse matrix.");
    pyend = mxGetPr(arg);*/
            ...
            plhs[8] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[8] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Rdark = mxGetPr(plhs[8]);
            
            plhs[9] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[9] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Gdark = mxGetPr(plhs[9]);
            
            plhs[10] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[10] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Bdark = mxGetPr(plhs[10]);
            
            plhs[11] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[11] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Rlight = mxGetPr(plhs[11]);
            
            plhs[12] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[12] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Glight = mxGetPr(plhs[12]);
            
            plhs[13] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[13] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            Blight = mxGetPr(plhs[13]);
            
            plhs[14] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[14] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            nxendd = mxGetPr(plhs[14]);
            
            plhs[15] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[15] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            nyendd = mxGetPr(plhs[15]);
            
            plhs[16] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[16] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            pxendd = mxGetPr(plhs[16]);
            
            plhs[17] = mxCreateDoubleMatrix(x_dim,y_dim,mxREAL);
            if (plhs[17] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            pyendd = mxGetPr(plhs[17]);
            
            
            //plhs[18] = mxCreateDoubleMatrix(1,4,mxREAL);
            //if (plhs[18] == NULL) mexErrMsgTxt("Cannot allocate result matrix");
            //position = mxGetPr(plhs[18]);
...
//g2thresh 	=  5.2* 0.0000596 * sensor_noise;
            find_edges(img,g1mag,g1dir,g1scale,g2,g2scale,g2all,sensor_noise,maxwidth,sublocflag,\
                    ...
                    imgR,imgG,imgB,Rdark,Gdark,Bdark,Rlight,Glight,Blight,nxendd,nyendd,pxendd,pyendd);
